...
class SnakeGameEnv:
    def __init__(self, frame_size_x=150, frame_size_y=150, growing_body=False):
    ...
    def get_state(self):
        ...
        # Get relative distances and discritezed distances
        ...
        state = (relative_horizontal, relative_vertical, discretized_distance_food)
        # Obtain the row number
        if self.game_over:
            print("dead")
            row = 27
        else:
            row = state[0] * 3 + state[1] * 3 + state[2] * 3
        return row
    ...
    def calculate_reward(self):
        ...
        if (snake_x > self.frame_size_x - 15 > food_x) or\
                ...
                (snake_x <15 and food_x >15) or (snake_y < 15 and food_y > 15):
            reward -= 80
